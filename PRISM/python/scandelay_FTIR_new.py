...
def accumulate_spectra_new(spectra, channel=1, apply_envelope=True, expand_envelope=1):

    try:
        return SpectralProcessor_new.accumulate_spectra(spectra,
                                                        channel=channel,
                                                    apply_envelope=apply_envelope,
                                                    expand_envelope=expand_envelope)
...
def average_spectrum_block(specblock1,specblock2):
    ...
    f=f2
    if not np.all(f1==f):
        s1 = SpectralProcessor_new.interpolate_spectrum(s1,f1,f)
        e1 = SpectralProcessor_new.interpolate_spectrum(e1,f1,f)
    ...
    leveler1 = SpectralProcessor_new.level_phase(f, s1*e1, order=1, manual_offset=0, return_leveler=True,
    ...
    leveler2 = SpectralProcessor_new.level_phase(f, s2*e2, order=1, manual_offset=0, return_leveler=True,
                                              weighted=True, subtract_baseline=True)
...
def heal_linescan(linescan):
    ...
    Nrows = SpectralProcessor_new.Nrows
...
def BB_referenced_spectrum_new(spectra,spectra_BB,
                           channel=1,channel_BB=1,
                          apply_envelope=True, envelope_width=1,
    ...
    try:
        SP=SpectralProcessor_new(spectra,spectra_BB,
...
def BB_referenced_linescan_new(linescan,spectra_BB,
                           channel=1,channel_BB=1,
                          apply_envelope=True, envelope_width=1,
                           smoothing=None,align_phase=False,
                           valid_thresh=.01):
...
def standardize_spectra_shape_new(spectra):

    # Actually, let us assume data may be coming in as shaped:
    ...
    spectra = np.array(spectra)
    assert spectra.ndim in (2,3),'`spectra` array must be 2D or 3D!'

    if spectra.ndim == 3:
        Nchannels2 = spectra.shape[1] - 1 - SpectralProcessor_new.Nrows_env
        assert (Nchannels2 % 2)==0, "If a 3D array, `spectra` must have an integral number of (amplitude,phase) channels!"
        return spectra

    # assume we are 2D
    Naccum = spectra.shape[0]/5
    assert Naccum == np.round(Naccum),'If `spectra is 2D, it should have shape (Naccumulations * 5, Nfreqs).'
    Nfreq = spectra.shape[1]
...
def normalized_spectrum_new(sample_spectra, sample_BB_spectra,
    ...
    sample_spectra = standardize_spectra_shape_new(sample_spectra)
    sample_BB_spectra = standardize_spectra_shape_new(sample_BB_spectra)
    ref_spectra = standardize_spectra_shape_new(ref_spectra)
    ref_BB_spectra = standardize_spectra_shape_new(ref_BB_spectra)

    try:
        SP = SpectralProcessor_new(sample_spectra, sample_BB_spectra,
                               ref_spectra, ref_BB_spectra,
...
def normalized_linescan_new(sample_linescan, sample_BB_spectra,
                        ref_spectra, ref_BB_spectra,
                        channel=1,channel_ref=1,channel_BB=1,
                        apply_envelope=True, envelope_width=1.5,
