...
class InterferogramAligner(object):
    ...
    alignment_p_filepath = "alignment_ps.txt"
    ...
    def regularized_intfg(self, intfgms_arr, fwd=True):
        ...
        p = np.polyfit(x=xnew, y=ynew, deg=6)
...
def align_interferograms(intfgms_arr, delay_calibration_factor=1,
                         order=1,refresh_alignment=True,Nwavelengths=4):
...
class SpectralProcessor(object):
    ...
    def process_spectrum(self,target='sample',
        ...
        f, spectra, spectra_BB = self.align_and_envelope_spectra(all_spectra, all_spectra_BB,
                                                                 apply_envelope=apply_envelope,
        ...
        if optimize_phase:
            ...
            spectrum = self.phase_aligned_spectrum(f, spectra, verbose=False)
            ...
            if view_phase_alignment:
                ...
                c = np.abs(spectrum)
                plt.plot(f, a, label='Phase discarded')
                plt.plot(f, b, label='Phase unaligned')
                plt.plot(f, c, label='Phase aligned')
                plt.ylabel('Power spectral density')
                plt.legend(loc='best')
                plt.twinx()
                plt.plot(f, a - c, color='r')
        ...
        f, spectrum_abs = self.normalize_spectrum(f, spectra,
                                              f, spectra_BB,
                                              valid_thresh=valid_thresh,
                                              abs=True,
                                              abs_ref=True) #discard phase of BB

        f, spectrum = self.normalize_spectrum(f, spectra,
                                              f, spectra_BB,
                                              valid_thresh=valid_thresh,
                                              abs=False,
                                              abs_ref=True) #discard phase of BB
    ...
    def __call__(self,
        ...
        return self.f_norm, self.norm_spectrum
...
def normalized_spectrum(sample_spectra, sample_BB_spectra,
    ...
    f, snorm = SP(apply_envelope=apply_envelope, envelope_width=envelope_width,
                  valid_thresh=valid_thresh,
                  optimize_BB=optimize_BB,
                  optimize_phase=optimize_phase,
                  view_phase_alignment=False)
...
def accumulate_spectra_old(spectra, phase_slope=0, apply_envelope=True,expand_envelope=1):
...
def normalize_spectra_old(spectra, spectra_ref, phase_slope=0,
                      apply_envelope=True, expand_envelope=1,
...
def double_normalize_spectra_old(spectra1, spectra1_ref,
    ...
    f1,s1_norm_abs,s1_norm_phase = normalize_spectra(spectra1, spectra1_ref, phase_slope=0,
                                                    apply_envelope=apply_envelope,expand_envelope=expand_envelope,
                                                     reference_phase=reference_phase)
    s1_norm = s1_norm_abs*np.exp(1j*s1_norm_phase)

    f2,s2_norm_abs,s2_norm_phase = normalize_spectra(spectra2, spectra2_ref, phase_slope=0,
                                                    apply_envelope=apply_envelope,expand_envelope=expand_envelope,
                                                     reference_phase=reference_phase)
